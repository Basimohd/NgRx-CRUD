...
import { UserAPISuccess, UserSaveAPISuccess, deleteUserSucces, invokeDeleteUser, invokeSaveUser, invokeUpdateUser, invokeUser, updateUserSuccessAPI } from "./users.action";
            ...
            switchMap((action) => {
                ...
                return this.UserService
                    .delete(action.id).pipe(
                        map((data => {
                            this.appStore.dispatch(setAPIStatus({ apiStatus: { apiResponse: '', apiStatus: 'success' } }))
                            return deleteUserSucces({ id: action.id })
                        }))
                    )
            })
        )
